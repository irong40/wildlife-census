...
7. Pass 2 waypoints are generated by a companion obstacle-avoidance pipeline: M4T rangefinder telemetry combined with OpenStreetMap line features (power lines, fences) and catenary inference for guy wires produces a confidence-tiered obstacle map (HIGH/MEDIUM/LOW) before any low-altitude investigation waypoint is executed. Operators resolve flagged hazards on the ground-station app before Pass 2 launches.
8. At mission end, a GeoJSON of all detections and a summary count report are available immediately — no post-flight analyst review required for routine surveys
